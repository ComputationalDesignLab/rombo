...
Some code has been adapted from the [BoTorch](https://botorch.org/) library and the adaptation of code has been indicated in the corresponding code files within the ``rombo`` folder. See BOTORCH_LICENSE file for licensing details of the BoTorch library code. Some code has also been adapted from the [blackbox](https://github.com/ComputationalDesignLab/blackbox) library and example scripts. See BLACKBOX_LICENSE for licensing details of the blackbox library code. 

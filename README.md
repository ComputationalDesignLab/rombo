...
project high dimensional outputs into a latent space. The latent space is modeled using multi-task Gaussian process (GP) models that utilize a Kronecker structure or intrinsic model coregionalization (ICM) formulation. The framework uses a Monte Carlo expected improvement infill strategy to balance exploration of the design space with exploitation of the objective function. A linear POD method is also implemented using the same structure as ROMBO but using POD for dimensionality reduction and independent GP models for the latent space. A standard BO implementation is also provided for generating comparison data for the ROMBO framework. Modular base classes have been provided for users to implement their own ROM architectures and utilize them within this framework. The framework is built utilizing [PyTorch](https://pytorch.org/) and associated libraries such as [GPyTorch](https://gpytorch.ai/) and [BoTorch](https://botorch.org/). Some code has been adapted from the [BoTorch](https://botorch.org/) library and the adaptation of code has been indicated in the corresponding code files within the ``rombo`` folder. See BOTORCH_LICENSE file for licensing details of the BoTorch library code. Some code has also been adapted from the [blackbox](https://github.com/ComputationalDesignLab/blackbox) library and example scripts. See BLACKBOX_LICENSE for licensing details of the blackbox library code. 

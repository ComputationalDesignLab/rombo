...
options = {
    ...
    "directory": "/scratch/negishi/adikshit/saasbo_infill_samples_opt{}_trial{}".format(args.optimizer, args.trial),
    "noOfProcessors": 64,
    "aeroProblem": ap,
...
# Defining bounds of the optimization and adding design variables to blackbox object
keys = ["upper", "lower"]
...
problem = InverseAirfoil(directory="/scratch/negishi/adikshit/50_samples_rae2822_HF_{}".format(args.trial), airfoil=airfoil, targetCp=gbo_cp, 
                    upper_bounds=upperBounds, lower_bounds=lowerBounds, normalized=True)

n_trials = 1
n_iterations = 75

# Defining arrays to store values in the optimization loop
...
boei_lengthscales = np.zeros((n_trials, n_iterations, 13))
romboei_lengthscales = np.zeros((n_trials, n_iterations, 13))

for trial in range(n_trials):

    # Defining the BO optimizers
    autoencoder = MLPAutoEnc(high_dim=problem.coefpressure.shape[-1], hidden_dims=[256,64], zd = 15, activation = torch.nn.SiLU())
    autoencoder.double()
...
savemat("./airfoil_inverse_design_saasbo_optimizer{}_trial{}.mat".format(args.optimizer, args.trial), results)

...
from rombo.optimization.stdbo import DKLBO
...
for trial in range(n_trials):
    ...
    for iteration in range(n_iterations):
        ...
        ti = time.time()
        optimizer1.do_one_step(tag = 'DKLBO + EI', tkwargs=optim_args)
        tf = time.time()
        boei_t[trial][iteration] = tf-ti
        ti = time.time()
        optimizer2.do_one_step(tag = 'DKLBO + Log EI', tkwargs=optim_args)
        tf = time.time()
        ...
        boei_EI[trial][iteration] = optimizer1.maxEI

        bologei_objectives[trial][iteration] = optimizer2.best_f
        bologei_dvs[trial][iteration] = optimizer2.best_x
        bologei_EI[trial][iteration] = optimizer2.maxEI
    
    boei_doe[trial] = optimizer1.xdoe.detach().cpu().numpy()
    bologei_doe[trial] = optimizer2.xdoe.detach().cpu().numpy()
...
#savemat("./env_model_results_{}_{}_DKLBO_trial_{}_LD_{}_MC_{}.mat".format(args.input_dim, args.output_dim, args.trial_num, args.latent_dim, args.mc_samples), results)

...
for trial in range(n_trials):
    ...
    autoencoder = MLPAutoEnc(high_dim=objective.output_dim, hidden_dims=[512,128,64], zd = args.latent_dim, activation = torch.nn.SiLU())
    autoencoder.double()
    rom_args = {"autoencoder": autoencoder, "low_dim_model": KroneckerMultiTaskGP, "low_dim_likelihood": ExactMarginalLogLikelihood,
                "standard": False, "saas": False, "ibnn": True, "ibnn_depth": 4}
    gp_args={"covar_module":InfiniteWidthBNNKernel(depth=4), "outcome_transform": Standardize(score_doe.shape[-1])}
